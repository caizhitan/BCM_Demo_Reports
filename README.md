      ...
      "values": [
        {
          ...
          "implementationPeriod": [
            ["27022024-31082024", "[102,25,25,33,0,0]"], // "StartDate-EndDate", Frequency of [MonToThurs,Fri,Sat,Sun,SchoolHoliday,Others] in between "27022024-31082024"
...
For `GRA/GCITC/GRA_BusFleet/GCITC_BusFleet` Conditions: (RouteDifference1 _ tripCount Array1) + (RouteDifference2 _ tripCount Array2) + as many values there are in the Group. Then use `totalAdditionalMileage` which sums the Arrays.
...
- Transaction
  ...
  For `all` conditions except `RA/CITC/GRA/GCITC` conditions: Get from API 
  ...
  ```
  For this case, its DiesalSD and DiesalDD.
...
- Handover/ReturnDate
  ...
  ```
  For this case, its 190224 and 280524.
...
- Total Freqency By Day Type:
  ...
  For `RA/CITC/RA_BusFleet/CITC_BusFleet` conditions: Add up `implementationPeriod` array.
  ...
  For `GRA/GCITC/GRA_BusFleet/GCITC_BusFleet` conditions: Add up all `implementationPeriod` array
  Example:

  ```JSON
  "implementationPeriod1": [
    ["27022024-31082024", "[102,25,25,33,0,0]"],
    ["01092024-31082025", "[198,52,52,63,0,0]"],
    ],

  "implementationPeriod2": [
    ["27022024-31082024", "[102,25,25,33,0,0]"],
    ["01092024-31082025", "[198,52,52,63,0,0]"],
    ],
  ...
  For `onlyBusFleet (NA)`: There is no need to calculate as `implementationPeriod` does not exist.

- Total mileage
  For `RA/CITC/RA_BusFleet/CITC_BusFleet` Conditions: Multiply `implementationPeriod` Array with additionalMileage Array. Then sum the numbers in `totalMileageForThePeriod` Array to give you mileage. Do this for all periods.
  ...
  "totalMileage1stPeriod": [[-61200, -3750, -5000, -825, 0, 0]]
  "mileage1stPeriod": -70775

  "totalMileage2ndPeriod": [[-118800, -7800, -10400, -1575, 0, 0]]
  "mileage1stPeriod": -138595

  "totalMileage": -209350
  ```

  For GRA/GCITC/GRA_BusFleet/GCITC_BusFleet Conditions: Multiply `implementationPeriod` Array with additionalMileage Array. Then sum the numbers in `totalMileageForThePeriod` Array to give you mileage. Do this for all periods and `implementationPeriod`'s.

  Example:

  ```JSON
  "1st_implementationPeriod": [
  ["27022024-31082024", "[102,25,25,33,0,0]"],
  ["01092024-31082025", "[198,52,52,63,0,0]"]
  ]

  "1st_additionalMileage": [-600, -150, -200, -25, 0, 0]
  "1st_totalMileage1stPeriod": [[-61200, -3750, -5000, -825, 0, 0]]
  "1st_mileage1stPeriod": -70775
  "1st_totalMileage2ndPeriod": [[-118800, -7800, -10400, -1575, 0, 0]]
  "1st_mileage1stPeriod": -138595

  "2nd_implementationPeriod": [
  ["27022024-31082024", "[102,25,25,33,0,0]"],
  ["01092024-31082025", "[198,52,52,63,0,0]"]
  ]
  "2nd_additionalMileage": [-600, -150, -200, -25, 0, 0]
  "2nd_totalMileage1stPeriod": [[-61200, -3750, -5000, -825, 0, 0]]
  "2nd_mileage1stPeriod": -70775
  "2nd_totalMileage2ndPeriod": [[-118800, -7800, -10400, -1575, 0, 0]]
  "2nd_mileage1stPeriod": -138595

  "totalMileage": -418700
  ```

  For `onlyBusFleet (NA)`: There is no need to calculate as `mileage` does not exist.

- Total Yearly SF Cost

  For `all` conditions except `onlyBusFleet (NA)`: (1stPeriod*mileage * 1stPeriod*BaseRate) + (XPeriod_mileage * XPeriod_BaseRate)

- Total Fuel Cost

  For `all` conditions except `onlyBusFleet (NA)`: (1stPeriod*mileage * 1stPeriod*FuelRate) + (XPeriod_mileage * XPeriod_FuelRate)

- Total SF Cost

  For `all` conditions except `onlyBusFleet (NA)`: (1stPeriod_YearlySFCost + 1stPeriod_FuelCost) + (XPeriod_YearSFCost + XPeriod_FuelCost)

- Total Cost (SF + LF)

  For `all` conditions except `onlyBusFleet (NA)`: (1stPeriod_leaseFeeValue + 1stPeriod_serviceFeeValue) + (XPeriod_leaseFeeValue \* XPeriod_serviceFeeValue).

  For `onlyBusFleet (NA)`: Use leaseFee values only. As no serviceFee values.

- Total Amount Required

  For `all` conditions: Use above TotalCost with Excel Formula: `=Ceiling(TotalCost,1000)`

- Annual Mileage
  For `all` conditions except `onlyBusFleet (NA)`: (TotalMileage/Sum of FrequencyByDayType) * 365 (Use bigDecimal Half-UP Rounding Method)
  ...
  "Sum of FrequencyByDayType": 1100,

  "annualMileage": 1659.09
  ```

  For `onlyBusFleet (NA)`: There is no need to calculate. As no `mileage` values.
- Annual SF
  For `all` conditions except `onlyBusFleet (NA)`: (1stPeriod_UnitRate * `annualMileage`)


  For `onlyBusFleet (NA)`: There is no need to calculate. As no `serviceFee` values.
- Annual LF

  For `all` conditions except `RA/GRA/CITC/GCITC` conditions: `leaseFee` * 12

- Annual Cost

  **No Formula** as of current writing.

## Our Data Objects
### For Variation Summary Report
- additionalMileage
- FirstReportData
- frequencyByDayType
- leaseFeeDetails
...
### For Service Mileage & Cost Summary Report 
- SecondReportData
- totalSecondReportData (sum of each period of SecondReportData)

### For Contract Variation Cost Summary Report
- ThirdReportData
- totalThirdReportData (sum of each period of ThirdReportData)
...
In summary this is how we use the calculated data in the different objects for Variation Summary Report.
